...
class ShippingMethodsService {
    ...
    getShippingMethodConditions(conditions) {
        return conditions
            ? {
                  ...
                  subtotal_min:
                      parse.getNumberIfPositive(conditions.subtotal_min) || 0,
                  subtotal_max:
                      parse.getNumberIfPositive(conditions.subtotal_max) || 0,
                  weight_total_min:
                      parse.getNumberIfPositive(conditions.weight_total_min) ||
                      0,
                  weight_total_max:
                      parse.getNumberIfPositive(conditions.weight_total_max) ||
                      0,
              }
            : {
                  countries: [],
    ...
    getValidDocumentForUpdate(id, data) {
        ...
        if (data.conditions !== undefined) {
            method.conditions = this.getShippingMethodConditions(
                data.conditions
            )
        }
